...
define HOST_PKGCONF_INSTALL_WRAPPER
	$(Q)$(INSTALL) -m 0755 -D package/pkgconf/pkg-config.in \
		$(HOST_DIR)/usr/bin/pkg-config
	$(Q)$(SED) 's,@PKG_CONFIG_LIBDIR@,$(STAGING_DIR)/usr/lib/pkgconfig:$(STAGING_DIR)/usr/share/pkgconfig,' \
		-e 's,@STAGING_DIR@,$(STAGING_DIR),' \
		$(HOST_DIR)/usr/bin/pkg-config
endef

define HOST_PKGCONF_STATIC
	$(Q)$(SED) 's,@STATIC@,--static,' $(HOST_DIR)/usr/bin/pkg-config
endef

define HOST_PKGCONF_SHARED
	$(Q)$(SED) 's,@STATIC@,,' $(HOST_DIR)/usr/bin/pkg-config
endef

...
ifeq ($(BR2_PACKAGE_QT_BUILD_FROM_TARBALL),y)
QT_SOURCE = qt-everywhere-opensource-src-$(QT_VERSION).tar.gz
QT_SITE = http://download.qt.io/archive/qt/$(QT_VERSION_MAJOR)/$(QT_VERSION)
else
QT_SITE = $(BR2_PACKAGE_QT_GIT_REPOSITORY)
# Since BR2_PACKAGE_QT_GIT_REVISION is a string type, its value shows
# up here with surrounding quotes. make, of course, doesn't strip
# those, but the shell does, leading to a mismatch between make's
# target and the actual on-disk file, breaking incremental builds.
# Just call subst to get rid of the extra quotes.
QT_VERSION = $(subst ",,$(BR2_PACKAGE_QT_GIT_REVISION))
QT_SITE_METHOD = git
endif
...
QT_LICENSE := LGPL-2.1 with exceptions
QT_LICENSE_FILES = LICENSE.LGPL LGPL_EXCEPTION.txt
...
ifeq ($(BR2_PACKAGE_QT_WEBKIT_VIDEO),y)
QT_CONFIGURE_OPTS += -DENABLE_VIDEO=1
else
QT_CONFIGURE_OPTS += -DENABLE_VIDEO=0
endif

ifeq ($(BR2_PACKAGE_QT_WEBKIT_WEB_AUDIO),y)
QT_CONFIGURE_OPTS += -DENABLE_WEB_AUDIO=1
else
QT_CONFIGURE_OPTS += -DENABLE_WEB_AUDIO=0
endif
...
define QT_INSTALL_STAGING_CMDS
	$(TARGET_MAKE_ENV) $(MAKE1) -C $(@D) install
